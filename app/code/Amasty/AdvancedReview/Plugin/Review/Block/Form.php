...
class Form
{
    ...
    private function getImageUploadHtml()
    {
        ...
        if ($this->blockHelper->isAllowGuest()) {
            $html = sprintf(
                '<div class="field row mt-3 mb-3 review-field-image %s">
                    <div class="col-12">
    ...
    private function getRecommendFieldHtml()
    {
        if ($this->configHelper->isRecommendFieldEnabled()) {
            $html = sprintf(
                '<div class="field amreview-recommend-wrap row mb-3">
    ...
    {
        $html = sprintf(
            '<div class="field review-field-email row">
                <div class="col-12">
                 <div class="control form-group">
                <label for="amreview-email-field" class="amreview-emailfield label">%s</label>
                <input id="amreview-email-field" 
                    class="form-control amreview-textfield input-text" 
                    type="text"
                    data-validate="{\'validate-email\':true}" 
                    name="guest_email"
                    data-bind="value: review().guest_email" /></div></div></div>',
            __('Email Address')
        );
