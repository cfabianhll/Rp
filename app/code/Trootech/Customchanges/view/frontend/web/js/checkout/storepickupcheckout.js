define(
    ...
    function ($, ko, quote, url, $t, mageTemplate, newMap , navigator, selectBillingAddress, checkoutData) {
        ...
        function getSelectListStoreHtml() {
            ...
            var $selectStoreHtml = $('<select class="list-store-select disable-ul" name="name_list"></select>');
        ...
        quote.shippingMethod.subscribe(function (value) {
            ...
            if((!$('.storepickup-information').length > 0)) $('#co-shipping-method-form').append(storePickupInformation);
            if (quote.shippingMethod()&&quote.shippingMethod().carrier_code == 'storepickup') {
                isShowShippingField(false);
                // $(".checkout-billing-address").hide();
                // $(".checkout-bottom-address-title").hide();
                ...
                if (!($('.list-store-to-pickup').length > 0)) {
                    ...
                    $('select[name=name_list] option:eq(1)').attr('selected', 'selected');
        ...
        function isShowShippingField(isShow) {
            var componentsToHide = [
                ...
                'shippingAddress.custom_attributes.identification_card',
                'shippingAddress.custom_attributes.corregimiento',
                'shippingAddress.country_id',
                'shippingAddress.postcode',
                'shippingAddress.region',
                'shippingAddress.region_id',
                'shippingAddress.city',
                'shippingAddress.custom_attributes.city_id'
            ];
        ...
        function fillStorePickUpAddressToField(data) {
            ...
            var mapComponentToData = {
                'shippingAddress.custom_attributes.corregimiento': 'demo',
                'shippingAddress.custom_attributes.identification_card': 'demo',
                ...
                'shippingAddress.custom_attributes.city_id': data.city,
                'street[0]': 'demo',
                'street[1]': '',
                'street[2]': '',
                'street[3]': '',
