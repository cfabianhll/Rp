...
var baseUrl = window.BASE_URL;
  $(document).on("change", "select[name='custom_attributes[corregimiento]']", function (e) {
  _data = jQuery("select[name='custom_attributes[city_id]']").find("option:selected").text()
  if(!doneTheStuff){
    doneTheStuff = true;
    $.ajax({
        url: baseUrl+'townshipcitymapping/index/collection',
        data: {
            city_id: _data,
        },
        type: "POST",
        ...
        if(data.data){
        jQuery("select[name='custom_attributes[corregimiento]']").empty();
        jQuery("select[name='custom_attributes[corregimiento]']").append(data.data);
        }
    });
  }
});

  $(document).on("change", "select[name='custom_attributes[city_id]']", function (e) {
    var _data = $(this).find("option:selected").text();
    console.log(_data);
    if(_data){
    $.ajax({
        url: baseUrl+'townshipcitymapping/index/collection',
  ...
  jQuery(document).ready(function(){
    ...
    var headTop = $(".mobile_display");

    headertopoption.on("scroll", function () {
      if (headertopoption.scrollTop() > 50) {
        headTop.addClass("mobile-header-none");
      } else {
        headTop.removeClass("mobile-header-none");
      }
    });

    
    jQuery(".showcart").click(function(){
        jQuery(".content-overlay").addClass("show-popup-cart");
    });
    jQuery("#content").click(function() {
      jQuery(".content-overlay").removeClass("show-popup-cart");
    });

    jQuery(".action-close").click(function() {
      console.log( jQuery(".action-close"))
      this.hide(); 
    });
  ...
  jQuery(".main_body_wrapper").append("<div id='left-menu-overlay' class='left-menu'>&nbsp;</div>");
  ...
  //alert("kkk65");
      jQuery(".filter-btn").click(function () {
        ...
        if(jQuery(".sidebar-col").hasClass("sidebar-open")) {
          ...
          jQuery("#left-menu-overlay").addClass("left-menu-overlay");
        }
    });
    
        jQuery(".close_menu").click(function () {
          
            jQuery("#left-menu-overlay").removeClass("left-menu-overlay");
            jQuery(".sidebar-col").removeClass("sidebar-open");
            jQuery(".sidebar-col").addClass("gw-sidebar");
      });
...
// jQuery(".text-md-right").click(function () {
//   jQuery(".sidebar-col").removeClass("sidebar-open");
// });
  var highestBox = 0;
...
jQuery(document).ready(function(){
  ...
  //Display salable qty
  if(jQuery('body.catalog-product-view').length){

    var prodViewId = jQuery("#product_addtocart_form").attr("data-product-sku");
  
    if(prodViewId){

      jQuery.ajax({
          url: window.BASE_URL+'salableqty.php',
          data: {
              prod_id: prodViewId,
          },
          type: "POST",
          ...
          if(data > 0){
            jQuery('.tst_link').text('En inventario');
            jQuery('.tst_link').show();
          }else{
            jQuery('.tst_link').text('Fuera de inventario');
            jQuery('.tst_link').show();
          }
      });
  ...
  const backLink = '<li class="nav-item"><a class="nav-link nav-back-link" href="javascript:;">Volver<i class="fas fa-chevron-left"></i></a></li>'
  ...
  $(ham).find('.fas.fa-times').hide();

  if(ham){
    ham.addEventListener('click', function() {
      if($(ham).find('.fas.fa-times').is(":hidden")){
        $(ham).find('.fas.fa-times').show();
        $(ham).find('.fa.fa-bars').hide();
      }else{
        $(ham).find('.fas.fa-times').hide();
        $(ham).find('.fa.fa-bars').show();
      }
  ...
  jQuery(".nav-link.nav-expand-link").on("click", function(){
    jQuery(this).parent().addClass("active");
  });
}
});
